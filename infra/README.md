...
### Step 5: Provision Lab Users (Optional)

```bash
ansible-playbook playbooks/05-provision-lab-users.yml -i inventory/k8s-cluster.yml
```

This provisions users from `users.csv`:
- Creates user accounts on the control plane node
- Sets up SSH authentication (from GitHub keys or CSV-provided keys)
- Copies kubeconfig to each user's home directory
- Sets KUBECONFIG environment variable in their .bashrc

The CSV is expected to have these fields after the header:
- Timestamp
- Has GitHub? (Yes!/No.)
- GitHub username
- Other username (if no GitHub)
- SSH public key (if no GitHub)
...
├── users.csv                        # Lab users (from Google Sheets)
├── inventory/
...
│   ├── 05-provision-lab-users.yml   # Provision users from CSV
│   ├── files/
